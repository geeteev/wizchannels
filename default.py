...
addon = xbmcaddon.Addon('plugin.video.wizchannels')
...
#else: SOURCES = []
SOURCES = [{"title": "wiz", "url": "https://pastebin.com/raw/Zib5i46S$$wizkey=wizzer$$", "fanart": "", "": "IPTV ", "date": "", "credits": "geetee", "thumbnail": ""}]

def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.wizchannels-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(Wizchannels,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(Wizchannels,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(Wizchannels,New source added.,5000,"+icon+")")
        if not url is None:
...
def getSoup(url,data=None):
        ...
        if url.startswith('http://') or url.startswith('https://'):
            ...
            if '$$wizkey=' in url:
                enckey=url.split('$$wizkey=')[1].split('$$')[0]
                rp='$$wizkey=%s$$'%enckey
                url=url.replace(rp,"")
...
def playmediawithproxy(media_url, name, iconImage,proxyip,port, proxyuser=None, proxypass=None): #jairox
    ...
    if media_url==None or media_url=='':
        xbmc.executebuiltin("XBMC.Notification(Wizchannels,Unable to play empty Url,5000,"+icon+")")
        return
        ...
        try:
            while player.is_active:
                ...
                if player.urlplayed==False and time.time()-beforestart>12:
                    print 'failed!!!'
                    xbmc.executebuiltin("XBMC.Notification(Wizchannels,Unable to play check proxy,5000,"+icon+")")
                    break
                #xbmc.sleep(1000)
        except: pass
...
def urlsolver(url):
    ...
    if host:
        ...
        xbmc.executebuiltin("XBMC.Notification(Wizchannels,Urlresolver donot support this domain. - ,5000)")
        resolver=url
    return resolver
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('Wizchannels','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('Wizchannels', 'Do you want to add this file as a source?')
        if ret:
            addSource(os.path.join(addon.getSetting('save_location'), name))

def _search(url,name):
   # print url,name
    pluginsearchurls = ['plugin://plugin.video.exodus/',\
             'plugin://plugin.video.NJMSoccer',\
             'plugin://plugin.video.salts/',\
             'plugin://plugin.video.MoneySports/',\
             'plugin://plugin.video.sanctuary/',\
             'plugin://plugin.video.SportsDevil/',\
             'plugin://plugin.video.adryanlist/',\
             'plugin://plugin.video.ccloudtv/',\
             'plugin://plugin.video.DCSports/',\
             'plugin://plugin.video.eyecandyTV/',\
             'plugin://plugin.video.gobble/',\
             'plugin://plugin.video.goodfellas/',\
			 'plugin://plugin.video.mic/',\
			 'plugin://plugin.video.prosport/',\
			 'plugin://plugin.video.specto/',\
			 'plugin://plugin.video.Vortex/'\
             ]
    names = ['Exodus','NJMSoccer','SALTS','MoneySports','Sanctuary','SportsDevil','Adryanlist',\
             'Ccloudtv','DCSports','EyecandyTV','Gobble','goodfellas','Made In Canada','ProSport','Specto','Vortex']
    dialog = xbmcgui.Dialog()
    index = dialog.select('Choose a video source', names)
...
#            print 'parentalblockedpin',parentalblockedpin
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('Remove from Wizchannels Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
            ...
            if not name in FAV:
                contextMenu.append(('Add to Wizchannels Favorites','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
#                print 'setting as folder in link'
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('Remove from Wizchannels Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('Add to Wizchannels Favorites','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
        try:
...
#            ln+='</item>'
        ...
        if url:
            ...
            xbmc.executebuiltin("XBMC.Notification(Wizchannels,Failed to extract regex. - "+"this"+",4000,"+icon+")")
elif mode==18:
    addon_log("youtubedl")
    try:
        import youtubedl
    except Exception:
        xbmc.executebuiltin("XBMC.Notification(Wizchannels,Please [COLOR yellow]install Youtube-dl[/COLOR] module ,10000,"")")
    ...
    if not (keyboard.isConfirmed() == False):
        ...
        if newStr==parentalblockedpin:
            ...
            xbmc.executebuiltin("XBMC.Notification(Wizchannels,Parental Block Disabled,5000,"+icon+")")
        else:
            xbmc.executebuiltin("XBMC.Notification(Wizchannels,Wrong Pin??,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
elif mode==56:
    addon_log("disable lock")
    addon.setSetting('parentalblocked', "true")
    xbmc.executebuiltin("XBMC.Notification(Wizchannels,Parental block enabled,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
